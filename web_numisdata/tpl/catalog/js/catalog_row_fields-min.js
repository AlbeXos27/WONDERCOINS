	...
	draw_item : function(item) {
		...
		switch(term_table){
			...
			case "mints":
				...
				const fila = filas_sin_inicio[0];
				
				if (fila && fila.term_section_label && fila.term_section_label.length > 0 && !this.rows_painted.includes(fila.section_id)) {
					console.log("filas_sin_inicio:", fila);
					this.rows_painted.push(fila.section_id);
					common.create_dom_element({
						element_type: "div",
						class_name: "mint",
						text_content:
							fila.term_section_label[0].startsWith("Grupo")
								? "(" + fila.term + ") " + item.term
								: item.term,	
						parent: fragment
					});

					if (item.term_section_id && fila.term_section_label[0] == "Cecas") {
						common.create_dom_element({
							element_type: "a",
					...
					console.warn("⚠️ No se encontró fila o term_section_label vacía para:", item);
				}
