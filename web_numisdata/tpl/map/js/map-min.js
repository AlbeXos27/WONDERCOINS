                ...
                .then((response)=>{
                        ...
                        if (response.result) {
                                ...
                                // clean empty geolocation items
                                        ...
                                        console.log("map points", map_points);
                                        
                                        /*SPECIALTOOLS*/

                                        for (let index in map_points) {
                                            
                                            const _data = map_points[index].data;
                                            const _geojson = map_points[index].geojson[0].layer_data.features;
                                            
                                            for (let i in _geojson) {
                                                
                                                Object.assign(_geojson[i].properties, _data);

                                                const map_global_data = self.current_map_global_data.find(function(el){

                                                    return el.section_id==_geojson[i].properties.section_id;

                                                });

                                                self.load_map_selection_info(_geojson[i].properties.section_id, map_global_data)
                                                .then(function(response) {

                                                    if (response) {

                                                            if (!_geojson[i].properties.hasOwnProperty('images')) {
                                                                
                                                                _geojson[i].properties.coins_rows = {};

                                                            }

                                                            _geojson[i].properties.coins_rows = response.coins_rows;
                                        ...
                                        if (response) {
                                                ...
                                                // activate images lightbox
                                                        setTimeout(function(){
                                                                ...
                                                                //console.log(images_gallery_containers);
