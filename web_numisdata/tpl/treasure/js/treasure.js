	...
	*/
	set_up : function(options) {
		...
		// options
			...
			self.row_detail_container	= options.row_detail
			self.section_id				= options.section_id
			self.table					= options.table

        console.log("self.section_id:",self.row_detail_container);
		if (self.section_id) {
			...
			// search by section_id
				...
				.then(async function(ar_rows){
					...
					if (ar_rows && ar_rows.length>0) {
						...
						// row render
							const hoard_node =await render_hoard.draw_treasure({
								row : self.row
							})
